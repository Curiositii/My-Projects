...
public class AddPartController {
    ...
    public String showPartFormForUpdate(@RequestParam("partID") int theId,Model theModel){
        ...
        List<Dessert> outsourcedParts=outsourcedrepo.findAll();
        for(Dessert outsourcedPart:outsourcedParts) {
            if(outsourcedPart.getId()==theId)inhouse=false;
        }
        String formtype;
        if(inhouse){
            Snack inhousePart=inhouserepo.findById(theId);
            theModel.addAttribute("inhousepart",inhousePart);
            formtype="InhousePartForm";
        }
        else{
            Dessert outsourcedPart=outsourcedrepo.findById(theId);
